    ...
    <block v-for="(item, index) in list" :key="index">
      <view v-if="!item.isedit" class="bg-white">
        ...
        <view class="flex" style="height: 100rpx">
          <view
            class="flex-1 font flex align-center justify-center"
      ...
      <view v-else class="bg-white">
        ...
        <view class="flex" style="height: 100rpx">
          <view
            class="flex-1 font flex align-center justify-center bg-main text-white"
        ...
        success: (res) => {
          ...
          VodUploader.start({
            ...
            error: (result) => {
              ...
              let msg =
                result.data.code === 10002
                  ? '腾讯云点播子应用ID配置不正确，请修改自己部署的后端配置。（注意：老师提供的线上测试接口上传视频功能已关闭，请自己下载群文件完整后端代码跟着课程部署）'
                  : result.data.message;
              uni.showModal({
                title: '上传失败',
                content: msg,
